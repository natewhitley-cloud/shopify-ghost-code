...
#### Update 2026-09-19 — category is crowding fast (verified live)

Three more entrants beyond the 07-08 set. **All still 0 reviews** — the review race remains wide open.

| Competitor                                          | Status (verified 2026-09-19)                            | Method / stance                                                                                                                   | Notes                                                                                                                                                                          |
| --------------------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Residue ‑ Leftover Code Clean** (theme-residue-cleaner) | Launched **2026-07-03**, **$49/yr** (~$4/mo), 0 reviews | **Auto-remove** (one-click), backup-before-cleanup, reversible, draft preview; attributes code to specific apps                   | Speedy Bloom. The auto-remove path we deliberately avoid (Cleanify's delisting risk). Cheapest in category. This is the "Residue ~$4/yr-equiv" from the 09-18 pricing comp. |
| **ScriptSweep** (scriptsweep)                       | Launched **2026-07-01**, Free + paid at scriptsweep.xyz, 0 reviews | **Detection-only**, review-first (no live-theme edits); client + dev report modes; **claims page-weight impact** + cleanup complexity | Same detection-only stance as us. Its page-weight-impact claim hits our known gap (gc-rrh.6 BLOCKED — theme-file scanner has zero perf-measurement capability).             |
| **Upright Cleaner** (UprightApps)                   | **NOT findable on App Store under that name** (pre-launch / soft-launch / diff store name). Known only from forum disclosure 2026-08-26 | **Read-only signature scanner** that "matches your theme against known app signatures and verifies each app is really gone before flagging" | **Closest to OUR method yet** (signatures + app-gone verification + read-only). Doing forum content-marketing (posted a leftover-code guide to the Site Speed board, drawing peer replies). Re-verify for a real listing. |

Tactical implications: (1) **the forum content-marketing lane is being worked by competitors** — UprightApps posted a helpful how-to guide with soft disclosure (no link) and it's pulling engagement; community.shopify.com has no anti-promo rule (see 6c) so this is a repeatable play we should run first-and-better, leaning on filename-attribution + consequence-grouping + the AI/schema-mismatch angle none of them have. (2) **ScriptSweep's page-weight claim** re-raises the perf-measurement gap — worth watching whether merchants cite it; feasible cousin is gc-rrh.10 (lazy-load/LCP detector). (3) Detection-only is now a *crowded* stance (us + ScriptSweep + Upright), so our differentiation must be depth (attribution, AI/schema, duplicate-tracker) not the stance itself. (4) Still **nobody has reviews** — the moat is unchanged: get to meaningful review count first.
