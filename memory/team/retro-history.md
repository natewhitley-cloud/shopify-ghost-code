...
- Key insight (GC-07t): expiring offline tokens are BY DESIGN and self-healing; expired offline rows are the normal resting state, not stuck merchants. Do not diagnose auth from the Session table — read Railway logs. Health checks must mirror the exact guard (`refreshToken: null`), not a superset. Captured in global memory + BEAD-2.
- Beads still frozen (168-world lineage) — all 4 session outputs captured as BEAD-1..4 in memory/pending-beads, not bd. BEAD-3 + BEAD-4 done-in-commits.
...
- Key win (verify-don't-trust ×3 again): (1) confirmed the gate wasn't a masked false-green by reading the smoke job log; (2) caught a misleading-error bug in the SHA-poll refinement (degraded-but-live build would report "never reported SHA" instead of the real health reason) before commit; (3) resolved the imports.md-vs-linter dispute by RUNNING eslint --fix on a probe rather than trusting either the doc or the tester learning.
- /tend run: 5 agents assessed, files already well-maintained (no manufactured trimming). Real payoff was a rules-health catch — `.claude/rules/imports.md` (must-strength) documented parent-before-sibling, but the linter enforces sibling-first (verified empirically). Fixed the rule + added `npx eslint --fix` as authoritative guidance. This CLOSES the import/order friction first flagged in the session-17 retro (two commit bounces).
- Discoveries → global memory: Railway CLI deploys have no git SHA; imports.md was wrong; stale MEMORY.md "migrate on boot" index line corrected to preDeployCommand.
...
- Key security refinement (from docs, not assumed): plan_handle is an UNAUTHENTICATED URL hint — a merchant could self-grant via /app?plan_handle=Professional. Design: use its PRESENCE as a "reconcile now" trigger only; Admin API stays sole source of truth for the tier. Chosen over trusting the param value.
- Key decision (C1 over C2/C3 via AskUserQuestion): shipped the redirect fast-path + 1h reconcile backstop using GA-only APIs; deferred the Partner API activeSubscription migration because it's only in the 2026-07 RC + needs a new org-cred auth pattern. Surfacing the RC-not-GA tradeoff before coding avoided building on an unstable API.
