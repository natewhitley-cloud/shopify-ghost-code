...
- PLAN_AMOUNTS in subscription webhook must stay in sync with billing config in shopify.server.ts. If prices change, both need updating. (added: 2026-03-28, dispatch: GC-hsk)
...
- `replacementBehavior` is a static plan config property in shopify.server.ts billing config, not a per-request param on billing.request(). Set APPLY_ON_NEXT_BILLING_CYCLE on the target plan so downgrades defer to next billing cycle. (added: 2026-03-28, dispatch: GC-rc0)
- Billing page needs in-app downgrade buttons, not just upgrade. Shopify Admin only offers cancel, not plan-switch. Without downgrade buttons, merchants must cancel + resubscribe (losing trial days, temporary feature gap). Use billing.request() with replacementBehavior: APPLY_ON_NEXT_BILLING_CYCLE for downgrades so merchants keep features until cycle ends. Only show downgrade buttons when on a paid tier. (added: 2026-03-28, from: bot-analytics-cleanup-app billing review)
