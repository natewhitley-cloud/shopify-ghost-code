...
- `RAILWAY_GIT_COMMIT_SHA` is only populated for repo-linked deploys, NOT `railway up` CLI deploys (caused the reverted 22d9d90). SHA injection in CI must write a file into the build context before `railway up`, never rely on Railway runtime env vars. (added: 2026-07-07, dispatch: GC-59t)
- When a file is conditionally present at build time (CI writes it, local builds don't), add `RUN [ -f <file> ] || touch <file>` in the Dockerfile build stage before any runtime-stage `COPY --from=build` of it — COPY of a missing source fails the build. (added: 2026-07-07, dispatch: GC-59t)
- `railway up` respects `.gitignore` by default (it has a `--no-gitignore` opt-out) — a CI-injected build-context file (like `.deploy-sha`) must NOT be gitignored or it silently never reaches the Docker build. Use `.dockerignore` to control image contents; caught by batch-2 audit after GC-59t shipped it gitignored. (added: 2026-07-07, dispatch: GC-59t-audit)
