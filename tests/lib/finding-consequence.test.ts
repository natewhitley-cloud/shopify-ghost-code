 ...
 *   - Partition: PRIMARY lanes cleanly partition all 33 types (each in exactly one).
...
// ---------------------------------------------------------------------------
// Partition: primary lanes cleanly cover all 33 types
// ---------------------------------------------------------------------------

describe("typesForLane partition", () => {
  it("primary lanes partition all 33 types with no type in two lanes", () => {
...
describe("laneForType / CONSEQUENCE_MAP spot checks", () => {
  ...
  it("DANGLING_REFERENCE -> customers-see-it / act-now / agentic true", () => {
    const e = CONSEQUENCE_MAP[FindingType.DANGLING_REFERENCE];
    expect(e.primary).toBe("customers-see-it");
    expect(e.urgency).toBe("act-now");
    expect(e.agentic).toBe(true);
    expect(e.secondary).toEqual(["discoverability"]);
